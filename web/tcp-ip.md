...
Далее наш компьютер устанавливает соединение с портом 80 компьютера 23.54.123.1 и посылает пакет с запросом на получение страницы www.example.com.
...
Над транспортным уровнем находится прикладной уровень, на котором работают такие протоколы, как HTTP, FTP и пр. Например FTP и HTTP используют надежный протокол TCP, а DMS сервер работает на небезопасном UDP.